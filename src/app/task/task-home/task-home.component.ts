...
export class TaskHomeComponent implements OnInit {
  @HostBinding('@routerAnim') state: any;
  lists = [
    {
      id: 1,
      name: '待办',
  ...
  launchUpdateTaskDialog(item: any) {
    const dialogRef = this.dialog.open(NewTaskComponent, { data: { title: '修改任务', task: item } });
  }
  launchConfirmTaskDialog() {
    const dialogRef = this.dialog.open(ConfirmDialogComponent, { data: { title: '删除任务列表', content: '确认删除该任务列表吗？' } });
    dialogRef.afterClosed().subscribe(result => console.log('result::', result));
  }
  launchEditListDialog() {
    const dialogRef = this.dialog.open(NewTaskListComponent, { data: { title: '更改列表名称', content: '确认删除该任务列表吗？' } });
    dialogRef.afterClosed().subscribe(result => console.log('result::', result));
  }
  launchNewListDialog() {
